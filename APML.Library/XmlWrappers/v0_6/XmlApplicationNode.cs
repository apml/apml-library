...
namespace APML.XmlWrappers.v0_6 {
  public class XmlApplicationNode : XmlAPMLComponentBase, IApplication {
    private IDictionary<string, IGenericItem> mItems = null;

    public XmlApplicationNode(APMLFileBase pFile, XmlNode pNode) : base(pFile, pNode) {
    }
    ...
    public bool HasItem(string pElName, string pMatchKey) {
      using (OpenReadSession()) {
        XmlNode itemNode = Node.SelectSingleNode(pElName +
    ...
    private void EnsureItemCacheCreated() {
      ...
      using (OpenWriteSession()) {
        ...
        XmlNodeList itemNodes = Node.ChildNodes;
